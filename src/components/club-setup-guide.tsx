...
export function ClubSetupGuide({ initial, clubSlug }: { initial: { teams: number; connectedTeams: number; contactReady: boolean; safeguardingReady: boolean }; clubSlug: string }) {
  ...
  const centralPath = !baseDomain || browserHostname === baseDomain || browserHostname === `www.${baseDomain}` ? `/${encodeURIComponent(clubSlug)}` : "";
  useEffect(() => {
  ...
  const steps = useMemo<Step[]>(() => [
    { id: "teams", title: "Set up your teams", description: initial.teams ? `${initial.teams} team${initial.teams === 1 ? "" : "s"} added${initial.connectedTeams ? ` · ${initial.connectedTeams} connected to FA Full-Time` : ""}.` : "Add your first team. Connecting FA Full-Time is optional.", href: `${centralPath}/dashboard/teams`, complete: initial.teams > 0 },
    { id: "appearance", title: "Shape your website", description: "Choose your badge, theme, and homepage hero carousel.", href: `${centralPath}/dashboard/club-settings`, complete: Boolean(manual.appearance) },
    { id: "media", title: "Add your media", description: images ? `${images} image${images === 1 ? "" : "s"} in your library.` : "Upload a few images for your homepage and news stories.", href: `${centralPath}/dashboard/club-settings#media`, complete: images > 0 },
    { id: "contact", title: "Add club contact details", description: "Help visitors get in touch with the club.", href: `${centralPath}/contact?edit=1`, complete: initial.contactReady },
    { id: "safeguarding", title: "Publish safeguarding details", description: "Add your welfare officer contact and public documents.", href: `${centralPath}/safeguarding?edit=1`, complete: initial.safeguardingReady },
    { id: "article", title: "Publish your first article", description: articles ? `${articles} article${articles === 1 ? "" : "s"} ready.` : "Share a welcome message or your latest club news.", href: `${centralPath}/news?edit=1`, complete: articles > 0 },
  ], [articles, images, initial, manual, centralPath]);
