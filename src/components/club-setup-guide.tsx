...
export function ClubSetupGuide({ initial }: { initial: { teams: number; connectedTeams: number; contactReady: boolean; safeguardingReady: boolean } }) {
  ...
  const [manual, setManual] = useState<Record<string, boolean>>(() => {
    ...
    try { return JSON.parse(window.localStorage.getItem("final-third-setup") ?? "{}") as Record<string, boolean>; } catch { return {}; }
  });
  ...
  const steps = useMemo<Step[]>(() => [
    { id: "teams", title: "Set up your teams", description: initial.teams ? `${initial.teams} team${initial.teams === 1 ? "" : "s"} added${initial.connectedTeams ? ` · ${initial.connectedTeams} connected to FA Full-Time` : ""}.` : "Add your first team and connect it to FA Full-Time.", href: "/dashboard/teams", complete: initial.teams > 0 && initial.connectedTeams === initial.teams },
  ...
  function markComplete(id: string) {
    const next = { ...manual, [id]: true }; setManual(next); window.localStorage.setItem("final-third-setup", JSON.stringify(next));
  }
  function finish() {
