...
async function selectClub(request: Request, clubId: string, nextPath: string) {
  ...
  const destination = new URL(nextPath.startsWith("/") ? nextPath : "/dashboard", request.url);
...
export async function GET(request: Request) {
  ...
  return selectClub(request, clubId, url.searchParams.get("next") ?? "/dashboard");
}
