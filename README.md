...
Open `http://localhost:3000` for the Final Third landing page and account setup, or `http://montpellier-fc.localhost:3000` for the club’s public website. The landing page links to account creation; after setup, the new club opens on its registered `<slug>.localhost` host. Public pages include `/news`, `/teams`, `/fixtures`, and `/contact`. Dashboard sections use paths such as `/dashboard/players`, `/dashboard/payments`, and `/dashboard/settings`; the club switcher changes subdomains and opens the selected club’s dashboard. Unknown club hosts return not-found.
